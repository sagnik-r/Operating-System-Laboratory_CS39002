...
/* List of processes in THREAD_BLOCKED state, that is, processes
   ...
   their sleep time expires. They are stored is increasing order of 
	 their sleep time */
...
timer_sleep (int64_t ticks) 
{
  ...
  while (timer_elapsed (start) < ticks) 
	...
	//NEW VERSION
	// Reject 0 or negative sleep ticks ( sanity check )
  if (ticks<=0)
    return;
  int64_t start = timer_ticks ();
...
/* Function to compare the sleep times of 2 threads. */
static bool 
