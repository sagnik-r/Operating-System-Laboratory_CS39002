...
/* Solution Code */
void
thread_wake_up()// The function associated with wake_up thread
{
	for(;;)
	{
		unblock_threads_in_timer ();
		thread_block();
	}
}
...
unblock_wake_up() 
{
	// wake_up thread to be unblocked so that sleeping threads can be woken up
...
thread_start (void) 
{
  ...
  /* Solution Code */
	struct semaphore wake_up_started;
	sema_init (&wake_up_started, 0);
	thread_create ("wake_up", PRI_MAX, thread_wake_up, &wake_up_started);
